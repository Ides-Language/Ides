...
namespace AST {
    ...
    llvm::Value* ASTIfStatement::GetValue(ParseContext& ctx) {
        ...
        llvm::Value* cond = this->condition->GetValue(ctx, Ides::Types::Integer1Type::GetSingletonPtr());
    ...
    llvm::Value* ASTWhileStatement::GetValue(ParseContext& ctx) {
        ...
        try {
            body->GetValue(ctx);
            cond = this->condition->GetValue(ctx, Ides::Types::Integer1Type::GetSingletonPtr());
            ctx.GetIRBuilder()->CreateCondBr(cond, whileblock, resumeblock);
        } catch (const Ides::AST::UnitValueException& ex) {
            // Block early-exits function.
        }
    ...
    llvm::Value* ASTForStatement::GetValue(ParseContext& ctx) {
        ...
        llvm::Value* endcond = this->endexpr->GetValue(ctx, Ides::Types::Integer1Type::GetSingletonPtr());
        ...
        try {
            ...
            endcond = this->endexpr->GetValue(ctx, Ides::Types::Integer1Type::GetSingletonPtr());
            ctx.GetIRBuilder()->CreateCondBr(endcond, forblock, resumeblock);
        } catch (const Ides::AST::UnitValueException& ex) {
            // Block early-exits function.
        }
